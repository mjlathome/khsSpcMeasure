...
public class PieceDetailActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			...
			arguments.putLong(DBAdapter.KEY_PROD_ID, getIntent()
					.getLongExtra(DBAdapter.KEY_PROD_ID, -1));
			PieceDetailFragment fragment = new PieceDetailFragment();
			fragment.setArguments(arguments);
			getFragmentManager().beginTransaction()
					.add(R.id.piece_detail_container, fragment).commit();
		}
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (id == android.R.id.home) {
			...
			//
			navigateUpTo(new Intent(this, PieceListActivity.class));
			return true;
		}
